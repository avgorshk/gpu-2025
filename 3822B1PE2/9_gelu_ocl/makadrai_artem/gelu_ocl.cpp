...
std::vector<float> GeluOCL(const std::vector<float> &input, int platform) {
  ...
  if (!initialized || cached_platform != platform) {
    ...
    cl_device_id device;
    clGetDeviceIDs(platforms[platform], CL_DEVICE_TYPE_GPU, 1, &device,
                   nullptr);

    context = clCreateContext(nullptr, 1, &device, nullptr, nullptr, nullptr);
    queue = clCreateCommandQueue(context, device, 0, nullptr);

    program = clCreateProgramWithSource(context, 1, &gelu_kernel_source,
                                        nullptr, nullptr);
  ...
  clEnqueueWriteBuffer(queue, d_input, CL_FALSE, 0, bytes, input.data(), 0,
                       nullptr, &write_event);
  ...
  cl_event read_event;
  clEnqueueReadBuffer(queue, d_output, CL_FALSE, 0, bytes, output.data(), 1,
